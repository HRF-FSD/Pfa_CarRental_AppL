...
class employee:
    ...
    def __init__(self):
        ...
        self.logout = Button(self.window1, text="Go Back", borderwidth=0, font=(14), bg="#2FB04A", fg="white",
                             command=self.logout)
        ...
        self.searchBtn = Button(self.searching,text="Search",bg="#3838e2",fg="white",width=20,cursor='hand2',command = self.searchClient)
        ...
        if item:
            try:
                ...
                if title == 'Get':
                    ...
                    if self.verif():
                        self.treeView.item(item, values=(lst[0],self.nom.get(),self.prenom.get(),self.age.get(),self.email.get(),self.tele.get(),self.adresse.get()))
                        tpl = (self.nom.get(), self.prenom.get(), self.age.get(), self.email.get(), self.tele.get(),self.adresse.get(),lst[0])
    ...
    def inserting(self):
        if self.verif():
            self.returnMaxid()
            i = self.max + 1
            self.Client.initialize(i, self.nom.get(),self.prenom.get(),self.age.get(),self.email.get(),self.tele.get(), self.adresse.get(),gestEmpl.selectUsername(config.username)[0][0])
            print(gestEmpl.selectUsername(config.username)[0][0])
