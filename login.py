...
class Login:
    def __init__(self):
        ...
        self.showPasswordImage = PhotoImage(file='eyeNotClicked.png')
        self.hidePasswordImgae = PhotoImage(file='eyeClicked.png')
        self.showPassword = Button(self.e2,image=self.showPasswordImage,bg="white",cursor='hand2',borderwidth=0,command=self.showPassword)
        self.showPassword.pack(side=RIGHT)
        self.clickIt = 0
        self.btn = Button(self.window, text="Login", borderwidth=0, font=(4, 10, "bold"), bg="#2FB04A", fg="white",command=self.button_clicked)
    ...
    def showPassword(self):
        if self.clickIt == 0:
            self.showPassword.configure(image=self.hidePasswordImgae)
            self.e2.configure(show='')
            self.clickIt=1
        else:
            self.showPassword.configure(image=self.showPasswordImage)
            self.e2.configure(show='*')
            self.clickIt = 0
